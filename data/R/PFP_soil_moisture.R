...
# UNT team.
unlink("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/UNT_MOISTURE_MONITORS/", recursive = T, force = T)
file.copy(from = normalizePath("/Volumes/USERS/Pueblo\ Farming\ Project/WEATHER/UNT_MOISTURE_MONITORS/"),
          to = normalizePath("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/"),
          overwrite = T,
          recursive = T)

read_moisture <- function(file, garden, location){
  cols <- readxl::read_excel(file, skip = 3) %>%
    ncol()
  if(cols == 7){
    out <- readxl::read_excel(path = file, skip = 3, col_names = c("Time","VWC_15","Temp_15","VWC_30","Temp_30","VWC_45","Temp_45"), 
                              col_types = c("date",rep("numeric",6)))
  }else{
    out <- readxl::read_excel(path = file, 
                              skip = 3, 
                              col_names = c("Time","VWC_15","Temp_15","VWC_30","Temp_30","VWC_45","Temp_45", paste0("X",1:(cols-7))), 
                              col_types = c("date",rep("numeric",cols-1)))
  }
...
cdg <- lapply(locs, function(loc){
  lapply(
    grep(loc,
         list.files("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/UNT_MOISTURE_MONITORS/Check_Dam_Garden",
                                               recursive = T,
                                               pattern = "xls",
                                               full.names = T),
         value = T),
...
mcg <- lapply(
  list.files("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/UNT\ Moisture\ Monitors/Coffey_Garden",
             recursive = T,
             pattern = "xls",
             full.names = T),
  read_moisture,
...
plc <- lapply(
  list.files("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/UNT\ Moisture\ Monitors/PLC_Garden",
             recursive = T,
             pattern = "xls",
             full.names = T),
  read_moisture,
