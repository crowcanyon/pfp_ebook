...
garden_locations <- lapply(list.files("../../DATA/Gardens/", pattern = "*.kml"), function(f){
  out <- sf::st_read(dsn=paste0("../../DATA/Gardens/",f),layer=f) %>% sf::st_zm() %>% as("Spatial")
    # rgdal::readOGR(dsn=paste0("../DATA/Gardens/",f),layer=f)
...
# unlink('./data/gardens')
sf::st_write(garden_locations %>% sf::st_as_sf(),
             dsn='./data/gardens.geojson',
             driver = "GeoJSON",
             delete_dsn = TRUE)
...
weather_station_IDs <- sf::st_read("./data/weather_stations.geojson") %>% as("Spatial")
weather_station_IDs <- weather_station_IDs@data %>%as_data_frame()
  # rgdal::readOGR(dsn = "./data/weather_stations.geojson", "OGRGeoJSON", verbose = FALSE, stringsAsFactors = F)@data %>%
  # as_data_frame()
