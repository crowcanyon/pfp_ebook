...
# UNT team.
file.copy(from = list.files("/Volumes/USERS/Pueblo\ Farming\ Project/WEATHER/CSV_WEATHER_STATIONS_AND_TEMPERATURE_PENDANTS/", pattern = ".csv", full.names = T),
          to = normalizePath("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/WEATHER_STATIONS/"),
          overwrite = F)
file.copy(from = "/Volumes/USERS/Pueblo\ Farming\ Project/WEATHER/PFP_weather_stations.csv",
          to = normalizePath("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/"),
          overwrite = T)

weather_stations <- list.files("~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/WEATHER_STATIONS/", pattern = ".csv", full.names = T) %>%
  map(function(f){
...
cortez_weather <- c(FedData::get_ghcn_daily_station(ID="USC00051886",
                                                    ...
                                                    standardize = T, 
                                                    raw.dir = "~/Google Drive/CCAC/Research Institute/PROJECTS/PFP/data/GHCN"),
...
weather_stations %>%
  ...
  ggplot2::ggplot(aes(x = DATE, y = PRCP_IN)) +
  ggplot2::geom_line() +
  ggplot2::ylim(0,1)
