...
```{r growth-dropdown, echo=FALSE, warning=FALSE, message=FALSE, results = "asis"}
cat(
'<form name="change">',
'<select name="options" onchange="document.getElementById(\'youriframe\').src = this.options[this.selectedIndex].value">',
'<option value="./plots/growth_',max(seasons),'.html">Select a year to view the growth data:</option>',
paste0('<option value="./plots/growth_',rev(seasons),'.html">',rev(seasons),'</option>'),
'</select>',
'<iframe name="iframe" id="youriframe" src="./plots/growth_',max(seasons),'.html" height="400" width="100%" frameborder="0" scrolling="no"></iframe>',
sep = "\n"
)
